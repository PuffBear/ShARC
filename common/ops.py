...
def import_instance(es):
    ...
    C = np.float32(es['C'])  # <-- FIX 1
    P = [i for i in range(1, es['P']+1)]
    M = [i for i in range(es['M'])]

    dms = np.asarray(dist_edges_from_file(es), dtype=np.float32)  # <-- FIX 2

    es_req = es['req']
    es_req = np.vstack([[0]*6, es_req])
    edge_indxs = np.int32(es_req[:, :2])

    demands = np.asarray(es_req[:, 2], dtype=np.float32) / C      # stays float32
    clss = np.int32(es_req[:, 3])
    s = np.asarray(es_req[:, 4], dtype=np.float32)
    d = np.asarray(es_req[:, 5], dtype=np.float32)
